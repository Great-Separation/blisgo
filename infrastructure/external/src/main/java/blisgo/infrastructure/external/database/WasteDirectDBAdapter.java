...
public class WasteDirectDBAdapter implements ViewCountable {
    ...
    public boolean updatePopularity() {
        var sql = "UPDATE waste " + "JOIN (SELECT waste_id, NTILE(10) OVER (ORDER BY views) AS star FROM waste) AS w2 "
                + "SET popularity = w2.star " + "WHERE waste.waste_id = w2.waste_id";
        jdbcTemplate.update(sql);
        return true;
    }

    public List<Map<String, String>> findAll() {
        var sql = "SELECT waste.waste_id, name, picture, wh.hashtags " + "FROM waste " + "JOIN waste_hashtags AS wh "
